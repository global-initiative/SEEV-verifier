...
def load_verify_signature(json_p: Path) -> Tuple[List[bytes], List[bytes], EccKey]:
...
def verify_secret(g_1: EccPoint, g_2: EccPoint, r: Integer, v: Integer, R: EccPoint, Z: EccPoint) -> bool:
	# TODO: my best guess is that this is not doable without giving away the value of the vote V in the bulletin board
	# 		This could only apply to the user's own receipt.
	Z_prime = g_1*((r+v) % Nist256.order)  # TODO: I am not sure of what v is currently
	R_prime = g_2*(r)
...
def load_vote_proof(json_p: Path) -> Tuple[List[EccPoint], List[EccPoint], List[Integer], List[Integer],
...
def load_ballot_proof(json_p: Path) -> Tuple[List[EccPoint], List[EccPoint], List[List[EccPoint]], List[List[EccPoint]],
...
def load_tally_data(json_p: Path) -> Tuple[List[EccPoint], List[EccPoint], List[List[EccPoint]], List[List[EccPoint]], List[Integer], List[Integer]]:
	with open(json_p, "r") as fin:
		data = json.load(fin)
...
def main():
	...
	data = load_verify_signature(json_p); data = zip(*data[:2], itertools.repeat(data[-1]))
	res_signature: Tuple[bool, ...] = tuple( verify_signature(*d) for d in data )

	data = load_vote_proof(json_p); data = zip(*data)
	res_vote_proof: Tuple[bool, ...] = tuple( vote_proof(*d) for d in data )

	data = load_ballot_proof(json_p);	data = zip(*data)
	res_ballot_proof: Tuple[bool, ...] = tuple( ballots_proof(*d) for d in data )
	
	data = load_tally_data(json_p);	data = zip(*data)
